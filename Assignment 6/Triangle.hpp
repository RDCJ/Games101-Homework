...
{
    Intersection inter;
    // if the Intersection happened at the back of the triangle, it doesn't count.
    if (dotProduct(ray.direction, normal) > 0)
        return inter;
    double u, v, t_tmp = 0;

    // if the crossproduct of ray and e2 is vertical to e1, that means the ray is parallel to the triangle
    ...
    // (1-u-v), u, v are barycentric coordinates
    ...
    inter.distance = (inter.coords - ray.origin).len();
    inter.obj = this;
    inter.m = this->m;
    return inter;
}
